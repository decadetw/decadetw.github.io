...
import {Anchor, Button, Card, Carousel, Col, Divider, Flex, Image, Row, Space, Tag, theme} from 'antd';
...
const works_2027='「故宮南院 X 豪華朗機工」－中央控制軟體技術總監'
const works_2026='Visual Model Training(SacoMesure AI Model),Gxxgle Home-Connection calc algorithm, 「臺中國家歌劇院 - 最後一問 X 豪華朗機工」－中央控制軟體技術總監'
...
const works_2018='「臺中世界花卉博覽會」聆聽花開的聲音 Ｘ 豪華朗機工－中央控制系統建置 ,「GoGoRo新⾞發表會」硬體控制系統建置 ,「宜蘭蘭傳藝中心 」互動介⾯多點控制設計,「臺北小巨蛋CTC世界盃國際標準舞公開賽」RF無線控制設計'
...
const works=[
    {year:2027,list:works_2027.split(',')},
    {year:2026,list:works_2026.split(',')},
...
const StaticHTML = () => {
    ...
    return (<>
            ...
            >
                <Divider/>
                <Tag style={{fontSize:'2em'}}>歷年實績|Works </Tag>
                <Divider/>
                <Row>
                    <Col span={16}>
                        {works.map((e, i, a) => {
                            return <div key={`${e.year}${i}`} id={e.year} style={{ background: 'rgba(222,222,222,0.502)'}}>
                                <Tag color={'gblack'} variant={'solid'}>-----{e.year}-----</Tag>
                                <>
                                {e.list.map((e2,i2,a2)=>{
                                    return <Tag style={{fontSize:'1em'}} color={'gray'} key={`${e.year}${i2}`}><CaretRightOutlined />{e2}</Tag>
                                })}
                                </>
                            </div>
                        })}
